...
/// Initialize rolling file logging under `{data_dir}/mihomo-tray/logs/{file_stem}.log`.
pub fn init_logging(file_stem: &str) -> Result<()> {
    use std::fs;
    ...
    let log_dir = base.join("mihomo-tray").join("logs");
    fs::create_dir_all(&log_dir)
        .with_context(|| format!("create log dir {}", log_dir.display()))?;

    let log_path = log_dir.join(format!("{file_stem}.log"));
    let roller_pattern = log_dir.join(format!("{file_stem}.{{}}.log"));
